...
using Ashraful_CoreProject.ViewModels;
...
namespace Ashraful_CoreProject.Controllers
{
    public class OrderController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            try
            {
                lst = await _context.OrderMasts.ToListAsync();
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> Details(int id)
        {
            ...
            try
            {
                OrderMast model = await _context.OrderMasts.Where(a => a.Id == id).Include(a => a.OrderDetl).FirstOrDefaultAsync();
        ...
        {
            OrderMast order = _context.OrderMasts.Find(id);
            if (order == null)
            {
                return NotFound();
            }
            _context.OrderMasts.Remove(order);
        ...
        {
            OrderDetl order = _context.OrderDetls.Find(id);
            if (order == null)
            {
                return NotFound();
            }
            _context.OrderDetls.Remove(order);
            _context.SaveChanges();
            return RedirectToAction("Index");
        }
